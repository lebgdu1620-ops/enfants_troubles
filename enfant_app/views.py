...
def taches_enfant(request, enfant_id):
    ...
    taches = Tache.objects.filter(enfant=enfant).order_by('ordre')
    ...
    if request.method == 'POST':
        ...
        # Si toutes les tâches sont finies → récompense
        if not taches.filter(terminee=False).exists():
            ...
            return redirect('recompense', enfant_id=enfant_id)
...
def recompense(request, enfant_id):
    enfant = get_object_or_404(Enfant, id=enfant_id)
    return render(request, 'enfant_app/recompense.html', {'enfant': enfant})
